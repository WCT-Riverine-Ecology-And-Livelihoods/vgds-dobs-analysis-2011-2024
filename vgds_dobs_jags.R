...
vgds_df <- read.csv("Data/vgdsNov2015_counts_rivcovs.csv")
...
# bionomial model for detection 
  logit(p[i,1]) <- alphaA0 + alpha1 * boatspeed[i] + alpha2 * W[i] + 
                   alpha3 * G[i] + alpha4 * rivwidth[i] + alpha5 * fog[i]
  logit(p[i,2]) <- alphaB0 + alpha1 * boatspeed[i] + alpha2 * W[i] + 
                   alpha3 * G[i] + alpha4 * rivwidth[i] + alpha5 * fog[i]
...
", fill = TRUE, file = "vgds_dobs_allcovs.txt")
